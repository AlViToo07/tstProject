class CategoriesManager {  
    ...
    setupEventListeners() {  
        this.searchInput.addEventListener('input', this.filterCategories.bind(this));  
        this.prevButton.addEventListener('click', this.goToPreviousPage.bind(this));  
        this.nextButton.addEventListener('click', this.goToNextPage.bind(this));  
    }  

    async fetchCategories() {  
        try {  
            const response = await fetch('http://127.0.0.1:8000/api/categories'); // Ganti dengan URL API Anda  
            if (!response.ok) {  
    ...
    renderCategories() {  
        ...
        paginatedCategories.forEach(category => {  
            ...
            categoryElement.classList.add('bg-white', 'p-4', 'rounded-lg', 'shadow-md');  
    ...
    updatePagination() {  
        const totalPages = Math.ceil(this.filteredCategories.length / this.categoriesPerPage);  
